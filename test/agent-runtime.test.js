...
import { AGENT_ROLES, agentPrompt } from '../src/agent/prompts.js';
...
test('agent prompts are composed from role-specific named parts', () => {
  ...
  try {
    ...
    expect(planner.parts.map(part => part.name)).toEqual([
      'runtime', 'planner', 'role_catalog', 'dependencies', 'planner_cli', 'decisions', 'common_cli', 'completion',
    ]);
...
test('project and local prompt overlays are explicit and ordered', () => {
  const root = temp();
  try {
    ...
    const view = agentPrompt(config, 'worker');
    expect(view.parts.slice(-4).map(part => part.name)).toEqual([
      'project.common', 'project.worker', 'local.common', 'local.worker',
    ]);
    expect(view.text.indexOf('PROJECT COMMON')).toBeLessThan(view.text.indexOf('PROJECT WORKER'));
    expect(view.text.indexOf('PROJECT WORKER')).toBeLessThan(view.text.indexOf('LOCAL COMMON'));
    expect(view.text.indexOf('LOCAL COMMON')).toBeLessThan(view.text.indexOf('LOCAL WORKER'));
  } finally { fs.rmSync(root, { recursive: true, force: true }); }
});
