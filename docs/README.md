...
Lush 把 AI 工作组织成**被动的 Service 节点**与**会干活的 Task**。文档按三层组织，从「为什么」到「精确的接口面」再到「怎么改它」：
...
- [Service 与 Task 模型](./concepts/service-model.md)：两类实体、生命周期三态、task 树的四条规则、agent 与 task 的关系。
...
- [CLI 与 package.json scripts](./reference/cli.md)：命令树、命令总览、开发用的 `bun run` 入口、`--interactive` / `--dry-run`。
