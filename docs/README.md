...
Lush 把 AI 工作组织成**被动的 Service 节点**与**会干活的 Task**；人的输入是一条 **Intension**，由 SID 0 串行解析成工作。文档按三层组织，从「为什么」到「精确的接口面」再到「怎么改它」：
...
- [Service 与 Task 模型](./concepts/service-model.md)：两类实体、生命周期三态、task 树的规则、agent 与 task 的关系。
- [Intension：用户输入与解析](./concepts/intensions.md)：唯一的人入口、队列为什么串行、冲突怎么变成给用户的问题、状态与不变量。
- [生命周期与孤儿监督](./concepts/lifecycle-and-orphans.md)：节点结束、子节点收养、SID 0 的回收策略、删除。
- [Agent 后端与 Context](./concepts/agents.md)：谁在替 task 干活，每次 invocation 看到什么。

## 参考层

- [CLI 与 package.json scripts](./reference/cli.md)：命令树、命令总览、开发用的 `bun run` 入口、`--interactive`。
