...
本模式用于你长时间离开界面、希望已有开发任务继续推进的场景。入口在 **设置 → 托管模式**；实现与权限接缝见[模块地图](engineering/modules.md#托管模式接缝)。模式属于当前项目，由 daemon 执行，不依赖浏览器保持打开。
...
开启后左栏常驻状态及 **立即关闭托管模式** 按钮；关闭不需要再次确认。关闭后不再执行新决定，会停止未完成的管家调用，但不会撤销已提交的答案、已经开始的 Git 操作或停止所有普通开发调用。
...
```bash
bun run lush auto-manage on --mode recommended --budget 200000 --existing no --merge no --confirm
bun run lush auto-manage on --mode preferences --budget 200000 --existing yes --merge yes --confirm
bun run lush auto-manage status
bun run lush auto-manage off
bun run lush auto-manage choices
bun run lush auto-manage choices --before 123
```
...
```bash
bun run lush auto-manage resume
```

恢复只启动原本排队的任务，不重新开启托管模式；已失败／取消的任务仍需人工检查后显式 retry。
...
授权、预算和暂停状态写入项目 SQLite 的 meta；选择意图及结果使用既有 Event，不引入业务表。daemon 重启保留模式及累计用量，但中断的选择不自动重放，结果未知的 Git 操作标明需要人工检查。源任务删除不删除独立选择快照；显式清空整个项目仍会清除全部 Event，须先关闭托管模式并等待管家操作结束。
