...
- 数据库事务只覆盖 Candidate 状态与事件。Git merge 不伪装在事务里：`accept` 先进入不可取消的 `accepted` 决策边界，Workspaces 在既有串行边界交付固定 commit，再以 `integration_succeeded` / `integration_failed` 回报状态机。`accepted` 期间 reject / request_changes / supersede（包括 prepare 新版）全部拒绝。
...
- [x] `accepted` 与 Git 结算之间不允许用户冲突动作撤回决定，且并发失败没有 planner / 新 Candidate 副作用。
...
- `test/project/candidates.test.js` 覆盖迟到成功/失败回调不能复活 rejected / changes_requested / superseded Candidate、Git 落地失败回到 ready，以及接受等待 Git 队列时 reject / changes / prepare 的确定性拒绝和最终 Git/状态一致。
