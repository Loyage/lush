...
- [x] 理解定位并确认生命周期歧义
- [x] 确认终态父节点的活动直接子节点交 SID 0；保留原父关系和事件
- [x] 确认 Task 显式完成、中断 invocation 不自动重放
- [x] 编写 README、architecture、service-model、rpc 和任务文档
...
- [x] ServiceManager、生命周期、Template、孤儿收养
- [x] persistence / Service / 收养 / 重开数据库 smoke test
...
- [x] SIGKILL 遗留锁由 SID 存活检测回收；shutdown 回复先落盘再拆 socket
