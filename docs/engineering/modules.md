...
- `Project.showcaseEligibility(branch,baseline?,excludeTaskId?)` 是 graph 与创建/重试共用的准入读面；Git 校验与树解析由 Workspaces 提供。`graph.get` 的 branch 节点增量带 `showcase:{allowed,reason,latest_task_id,reserved,reserved_at,reserve_allowed,reserve_reason}`，前端按 `reserve_allowed` / `reserved` 在分支详情给「预约效果展示」/「取消预约」入口（入口不要求当前已满足完整准入，`allowed` 只影响确认文案与是否立即开始）；首页、Intent 列表和任务详情不提供启动入口。已有展示仍可查看。在独立 detached worktree 执行，不切换/提交用户分支；展示任务的工作区在 Task 图卡片、分支图任务行与任务详情里都显式标成 `detached worktree`，不冒充源分支检出。
