...
1. CLI RPC `call`（或 agent 工具 `task_spawn`）进入 `ServiceManager.spawnTask(...)`：它校验节点是否 active、这一个 service 上是否已有活动 task、目标是否是自己的直接子 service，然后建 task 行并在后台启动它的 run。
