...
| `render-overview.js` | 项目概览：指标按 Intent / 并行执行 / 效果展示 / 需要你决定计，提供选择本地分支启动展示、最近展示列表，候选同时显示 verification 的 pass / fail / partial / unverified / unknown；Intent 成果主线（含候选报告入口）先于折叠的 Git 交付诊断，任务只作明细；`kind='info'` 提醒、运行中 agent 与时间轴、维护信息照旧（`render-ladder.js` / `merge-select.js` 仍可用，但没有常驻视图） | `renderOverview(data)` |
