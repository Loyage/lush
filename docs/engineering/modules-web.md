...
| `composer.js` | 输入缓存与提交表单；默认折叠只留一行输入 + 一行操作（父分支字段与快捷键说明点开「展开」才出现，折叠态在控件上标出非空父分支；展开状态只在会话内）；输入框用 `.composer-input` + `.composer-highlight` overlay 就地高亮命中的快速路由前缀（读快照 `settings.input_routes.value`，与 core 同规则匹配，用 `textContent`/`createElement` 构造、不拼 innerHTML，随滚动同步偏移；overlay 置于透明背景 textarea 之下，文字与光标始终在上层，色块不会盖住前缀字符）；「全部执行」先暂存正文，再不带 `ids` 调用 `draft.commit` 逐条提交全部未提交草稿；汇总 `result.inputs` 里的快速路由命中（用 `input[route.target]` 取 worker/research，不假定 worker）与交给 planner 的条数；提示统一交给 `messages.js`，不再自己写输入栏底部的 `#error` | `buffer()`、`syncComposer()`、`paintInputHighlight()`、`paintDraftPanel()`、`toggleDraftPanel()`、`paintComposerDetails()`、`toggleComposerDetails()`、`initComposer()` |
| `input-routes.js` | 浏览器侧快速路由前缀匹配纯模块；与 `src/core/input-routes.js` 同规则的 `matchInputRoute` 及 `ROUTE_TARGETS` / `DEFAULT_INPUT_ROUTES`，供输入框高亮与设置页前缀编辑器使用；两侧行为由 `test/web/input-routes-parity.test.js` 用同一组用例锁住，改一侧规则会在测试里暴露 | `ROUTE_TARGETS`、`DEFAULT_INPUT_ROUTES`、`matchInputRoute(routes, content)` |
...
| `render-detail.js` | 任务详情整页：一句话短标题（`taskTitle`）、完整 goal 以 Markdown 正文排在结果之前、状态、结果优先的阅读顺序与任务操作；头部用 `roleBadge` 显示类型颜色，`task.route` 为真时另带「⚡ 快速路由」徽章 | `renderDetail(task, history, diff, usage)`、`renderDetailError(taskId, message)` |
