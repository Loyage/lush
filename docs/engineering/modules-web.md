...
新 say 的 `task_kind`、结构化 `reservation` 与 `parent_task_kind` 随 `graph.get` 的 task 节点透传，分支图任务行显示阶段、分支表头给预约/审批入口；新 say 分支不提供旧 `branch.merge/sync/catchup` 与旧 branch 级展示预约，避免误导；审批确认明示固定源 commit 与父 baseline；父分支被未集成请求锁住时，分支节点的 `freeze`（`kind='delivery'`）会禁用旧「一键合并全部子分支」并在提示里说清原因。两处复用 `render-delivery.js`。
