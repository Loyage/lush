...
| 文件 | 职责 | 导出 |
|---|---|---|
| `app.js` | 唯一入口：先经 `project-picker.js` 确认项目，再装配左栏顶部身份区按钮（品牌回概览 / 切换项目 / 移动端导航 / 右侧返回）、`#graph` / `#settings` / `#statistics` / 四个信息页 / 任务 / 文档的 hash 路由与两个定时器；定时器按「轮询频率」偏好重建；全局模式下另按 20s 低频刷新左栏项目列表摘要 | `boot()` |
...
| `composer.js` | 输入缓存与提交表单；默认折叠只留一行输入 + 一行操作（父分支字段与快捷键说明点开「展开」才出现，折叠态在控件上标出非空父分支；展开状态只在会话内）；发送按钮只提交输入框中的这条正文（`say.submit`，可带父分支），不连带发送缓存的草稿；**输入框不再做快速路由前缀高亮**：新 say 不走快速路由，高亮会让人误以为会按前缀派活；提示统一交给 `messages.js` | `buffer()`、`syncComposer()`、`paintDraftPanel()`、`toggleDraftPanel()`、`paintComposerDetails()`、`toggleComposerDetails()`、`initComposer()` |
...
| `render-intents.js` | Intent 列表：原始目标、planner 闸门、Plan 计数、最近展示任务的只读链接，以及历史 Review Candidate 的「打开结果 / 接受并合入 / 要求修改」动作；输入带 `route` 时行内加「⚡ 快速路由」徽章 | `renderIntents(data)` |
| `render-specs.js` | 拆解队列（只读） | `renderSpecs(data)`、`specItem(spec)`、`specDeps(value)` |
| `render-tree.js` | 全类型任务列表与任务树（含规划、历史调度、展示与执行介绍）；角色以 `roleBadge` 彩色胶囊显示，快速路由任务（`task.route`）整行加 `.route-flagged` 底色并在状态旁加 `routeBadge`；固定完整类型筛选、兄弟链、依赖标签、为什么没在跑；明确标出“活动 + 最近历史”的截断范围，通过 `/api/tasks?scope=all&before=` 按需加载更早页，筛选只针对已加载记录 | `renderTree(data)` |
...
| `render-delivery.js` | 新 say 在静息、有执行结果且有待交付提交时优先显示「请求合并」（仍经 `task.reserve` 发出固定提交请求，不自动批准）；展示交付并结算为 completed 的 say 无 pending 可等，同样显示「请求合并」直接补发一次固定提交请求（撤销后也保留该入口），未就绪时保留互斥展示/合并预约与撤销、pending 的显式安全复查（复用同类 `task.reserve`，让父分支交付锁之类阶段性阻塞可见）、父子分歧时用户确认后派源侧解分歧 Agent 子任务（终态 say 同样可派；不自动集成父分支，终态 say 的产物由 runtime 推进并重新发请求；终态未集成 child 引导检查并显式归档后新派，子 Task 详情不提供重放）、已发出未集成的请求显示失效诊断（`parent_moved` / `parent_locked` / `source_moved` / `contained`）、给只读「复查请求」（复用同类 `task.reserve`，会按当前 Git 事实重写诊断）并可「撤销请求」解除父分支交付锁（确认弹窗说明不删分支与提交）、冻结源 commit + 父 baseline 的用户批准；Task 详情/分支图复用同一组确认与按钮提示，展示预约用 `agent-call` + `agentHelp`；旧任务与非 say 子节点不显示 | `deliveryControls(task,{refresh})` |
| `render-detail.js` | 任务详情整页：一句话短标题（`taskTitle`）、完整 goal 以 Markdown 正文排在结果之前、状态、结果优先的阅读顺序与任务操作；头部用 `roleBadge` 显示类型颜色，`task.route` 为真时另带「⚡ 快速路由」徽章，`task_kind='analysis'` 另带「只读分析」徽章；main/owner 详情给「问这条分支」（`agent-call` + 弹窗确认）：问题经用户专属 `task.analyze` 启一个只读分析子 Task 并跳过去 | `renderDetail(task, history, diff, usage)`、`renderDetailError(taskId, message)` |
| `render-overview.js` | 项目概览：指标按 Intent / 并行执行 / 需要你决定计，不提供展示启动按钮、指标或大列表，仅保留关联 Intent 已有展示的查看链接，候选同时显示 verification 的 pass / fail / partial / unverified / unknown；Intent 成果主线（含候选报告入口）先于折叠的 Git 交付诊断，任务只作明细；`kind='info'` 提醒、运行中 agent、维护信息照旧（`render-ladder.js` / `merge-select.js` 仍可用，但没有常驻视图） | `renderOverview(data)` |
| `task-graph-layout.js` | Task 父子森林纯逻辑：缺失父节点作为可见根，坏数据成环不死循环 | `taskForest(graph)` |
| `render-task-graph.js` | `#task-graph` 独立视图：Task 卡片展示目标、结果、等待原因、执行进度、待决、交付与当前 Git 诊断；卡片按真实状态配色（running 呼吸外环，其余各一色，中性 idle 兜底），表头汇总行以同一套状态色列计数兼作图例；有子 say 分支的 Task 给「编排合并全部子 Task」入口（目标就是 Task 自己的分支），点击先拉只读计划再确认，运行中改显进度与取消——编排计划、确认与执行复用 `render-graph.js` 导出的同一份实现；复用分支图待决/诊断组件与任务详情交付按钮；折叠按项目存、输入草稿不被轮询覆盖；保留旧分支图入口 | `openTaskGraph()`、`loadTaskGraph()`、`renderTaskGraph(graph)` |
...
Task 图走 `task.graph` / `/api/task-graph`、`#task-graph`；分支图仍走 `graph.get` / `#graph` 且原 Git 操作完整保留。参见 [Task 图与固定输入规则](task-graph.md)。

新 say 的 `task_kind`、结构化 `reservation`、`parent_task_kind` 与 `has_result`（仅表明已有一次执行结果，不是验收结论）随 `graph.get` 的 task 节点透传，分支图任务行显示阶段、分支表头给预约/审批入口；新 say 分支不提供旧 `branch.merge/sync/catchup` 与旧 branch 级展示预约，避免误导；展示交付后的 completed say 由 `render-delivery.js` 补上「请求合并」入口，而不是恢复旧 `branch.merge`；该入口遇分歧会留下 pending/diverged 预约并继续给「派子任务解决分歧」，完成后由 runtime 收尾；审批确认明示固定源 commit 与父 baseline；父分支被未集成请求锁住时，分支节点的 `freeze`（`kind='delivery'`）会禁用旧「一键合并全部子分支」并在提示里说清原因。两处复用 `render-delivery.js`。
...
| 文件 | 职责 | 导出 / 接缝 |
|---|---|---|
| `src/ui/web/server.js` | 单项目与全局工作台两种 HTTP host；资源、认证、窄 API 路由、按项目索引的连接管理（single-flight）与 `/p/<project-id>/` 项目身份路由；保留 `/api/snapshot`，新增 `/api/overview`、`/api/tasks`、事件历史页与设置页 Agent 配置按需路由 | `startWeb()`、`createProjectHost()`、`rememberWebProject()` |
