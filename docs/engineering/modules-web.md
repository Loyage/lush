...
| `render-task-graph.js` | `#task-graph` 独立视图：Task 卡片展示目标、结果、等待原因、执行进度、待决、交付与当前 Git 诊断；复用分支图待决/诊断组件与任务详情交付按钮；折叠按项目存、输入草稿不被轮询覆盖；保留旧分支图入口 | `openTaskGraph()`、`loadTaskGraph()`、`renderTaskGraph(graph)` |
