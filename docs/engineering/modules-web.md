...
| `composer.js` | 输入缓存与提交表单；默认折叠只留一行输入 + 一行操作（父分支字段与快捷键说明点开「展开」才出现，折叠态在控件上标出非空父分支；展开状态只在会话内）；输入框用 `.composer-input` + `.composer-highlight` overlay 就地高亮命中的快速路由前缀（读快照 `settings.input_routes.value`，与 core 同规则匹配，用 `textContent`/`createElement` 构造、不拼 innerHTML，随滚动同步偏移；overlay 置于透明背景 textarea 之下，文字与光标始终在上层，色块不会盖住前缀字符）；「直接执行」与「提交并规划」都感知 `result.route`，命中时显示「前缀 X 命中，已创建 worker/research #N」而不是假定 worker；提示统一交给 `messages.js`，不再自己写输入栏底部的 `#error` | `buffer()`、`selectedDraftIds()`、`syncComposer()`、`paintInputHighlight()`、`paintDraftPanel()`、`toggleDraftPanel()`、`paintComposerDetails()`、`toggleComposerDetails()`、`initComposer()` |
