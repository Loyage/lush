...
默认表：`开发` → worker（可写代码），`解释` → research（只读调研，不创建分支 / worktree）。前缀表是项目级运行设置 `input_routes`（`<home>/settings.json`，`system.configure` 读写），热生效，无需重启；缺省时回退到默认表，写 `null` 清除覆盖。每项恰为 `{prefix,target}`，prefix 非空、≤32 字符且不含空白，target 只能是 worker / research，prefix 大小写不敏感去重，最多 32 项。

命中时 planner 在同一事务里写 `input.route` 事件（含 prefix / target / task / spec），随后标 completed；`request` 结果带 `result.route = {prefix,target}`，`target=worker` 时给 `result.worker`，`target=research` 时给 `result.research`；`inputs()` 读模型用 `route` 布尔字段标出这类输入。
