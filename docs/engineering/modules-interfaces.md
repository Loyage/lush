...
| `rpc/handlers/task.js` | `task.*`（`task.retry(id,profile?)` 可带仅本轮生效的完整 Agent Profile；旧 `task.list/history` 保留；新增 `task.activity`、`task.page`、`task.history_page` 有界读接口，以及用户专属的 `task.transcript_search` / `task.transcript_step`）、用户专属 `explanation.start/list/get`、agent-only 的 `progress.plan` / `progress.complete` | `handlers` |
