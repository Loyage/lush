...
| `rpc/handlers/system.js` | `system.*`（含用户专属的 `system.configure`）、`graph.get`、Agent 配置与资源接口；环境文件的 `agent.environment` / `agent.environment.configure` 因可能含密钥，读写都为用户专属 | `handlers` |
