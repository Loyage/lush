...
CLI 的 task list / history 支持 cursor 分页；task inspect 返回完整任务结果和有界的相关记录。Web 复用 UIClient，轮询快照，采用 textContent 呈现模型输出，不插入 HTML；输入表单、尚未加入草稿的上下文引用和 notice 答复在轮询时保留。

Web 在任务、任务子树、交付分支、意图、spec、notice、diff、结果、消息、执行步骤、事件和检验记录上注册语义引用；其它页面文字可直接选中后引用。只有存在候选时才拦截 `contextmenu`，否则保留浏览器原生菜单。输入框上方的引用卡片可删除；加入草稿后引用持久化并随批量提交按段落进入 Input。所有引用文字仍以 `textContent` 渲染。
