...
- **daemon 是长驻服务，版本在启动瞬间冻结**：`src/agent/guide.js`（喂给 agent 的提示词）、`src/cli/tree/`（CLI 声明树）、`templates/**/*.json` 都在启动时读入内存。改了这些，不 restart 不生效；改运行期代码（`src/core/`、`src/daemon/`、`src/persistence/`）同理。
