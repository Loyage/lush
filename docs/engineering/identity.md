...
fingerprint 只哈希 `src/agent/guide.js`、`src/cli/main.js`、`src/cli/tree/*.js`（CLI 声明树）与 `templates/**/*.json` 和 `templates/**/*.md`（递归，含嵌套子目录；`.md` 是模板散文字段的 `@` 引用文件）。所以 `cli.code_match: true` 仅表示「**喂给 agent 的提示词与 CLI 声明面**」一致，**不代表整个代码库一致**：改 `src/core/`、`src/daemon/`、`src/persistence/` 等运行期代码不会改变 fingerprint，但行为会变。**改任何运行期代码后同样要 restart daemon。**
