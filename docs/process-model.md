...
注入创建方 Agent 的 `available_child_templates` 只列**此刻真能创建成功**的模板：按创建时快照的 `child_templates` 过滤权限、排除 `lush-root`，并剔除 `singleton: true` 且本 PID 下已有活动实例的模板（否则调用方只会白撞一次 `process.spawn` 的拒绝）。它是派生视图，不代替权限本身：完整白名单仍在 `child_templates`，被占位的那个实例就在 `children` 里。列表顺序是层级序：能创建某个模板的模板排在它前面（`*` 与自引用不算层级关系），同级按名称排序，与文件名字母序无关。

限制针对 template 名称，而非 task/service：模板文件里 `child_templates` 写的是相对自己文件的路径，loader 加载时解析成模板名后存快照，下面的限制与比对全部按名字进行。`["*"]` 表示允许全部已加载模板；`[]` 禁止创建子节点。父节点必须 running。自身模板快照中的 `child_templates` 决定后续创建权限，外部文件修改不会悄悄改变已有进程能力。**唯一的例外是 PID 0**：它的快照在每次 daemon 启动时从当前加载的 `lush-root` 刷新（只 PID 0，有差异时记 `template_refreshed` 事件），所以收窄 / 放宽根权限靠改 `templates/lush-root.json` + 重启 daemon 生效。变量值（`state.params` 与 `state.vars`）属于 Context state，但只有变量的写入路径能改它；其余 state 由 agent 自己维护。子节点 goal 由创建参数指定，未指定时使用名称；模板本身不携带初始 Context，新实例的 artifacts、references 一律从空开始，state 里只有它的变量（immutable → `state.params`，mutable → `state.vars`）。ContextBuilder 提供父节点摘要。

模板快照是创建时的完整副本（含 `singleton`、`spawn_prompt`、`variables`），外部改模板文件不会改变已有 Process。`singleton` 与 `type` 按当前已加载模板判定；例外是从快照读回的那两个字段：daemon 启动时会把旧快照中缺失的 `child_templates` 与 `variables` 从同名已加载模板回填一次（幂等，记 template_backfilled 事件，不改其他字段），同名模板已不存在时跳过并记日志。第二个例外是 PID 0：daemon 启动时会用当前加载的 `lush-root` **整份替换**它的快照（`Repository.replaceSnapshot` + `ProcessManager.refreshRootTemplate`：逐字段比较，有差异才写，记 `template_refreshed` 事件，`data.fields` 按稳定顺序列出变化的键）；这与 `template_backfilled` 的「只补缺失字段」不同，是整份替换，且只作用于 PID 0。因此根权限跟随模板文件 + 重启 daemon，而其它进程仍保持创建时的能力。旧快照里遗留的 `process_type`、`allowed_child_templates`、`agent_command`、`initial_context` 键不再被读取。

仓库顶层 `templates/` 内置 `lush-root.json`（PID 0 专用，`child_templates` 只有 `project-manager`，任何节点都不能用它 spawn 子进程），其余模板按 spawn 树嵌套摆放：`<name>.json` 旁边有一个同名文件夹 `<name>/`，里面是它能直接创建的模板（共享模板跟最浅的父模板放一起）——`lush-root/project-manager.json`、`lush-root/project-manager/{project.json,generic-task.json,research-task.json,generic-service.json}`、`lush-root/project-manager/project/dev-task.json`、`lush-root/project-manager/project/dev-task/worktree-service.json`——每个模板的 `child_templates` 写相对自己文件的路径（`project/dev-task.json`、`dev-task/worktree-service.json`、同目录的 `generic-task.json`），由 loader 递归加载并解析回模板名。其余模板需自行编写，放入 `templates/` 或 `$LUSH_HOME/templates/`（同样递归，相对路径相对自己的文件）；用户目录模板不能覆盖仓库模板名称；PID 0 也不会创建用户模板（`child_templates` 是显式列表，不再有 `*`）；loader 检查 `type`、`singleton`、必填字段和列表中的模板引用（路径指向的文件不存在时直接报错）。

`dev-task`（`project` 的子模板、task、非单例、`child_templates: ['worktree-service']`）把一项开发任务描述成三个正式字段，都是它自己声明、受校验、可渲染的变量：`name`（不可留空的英文标识符 `^[A-Za-z][A-Za-z0-9_-]*$`、≤64、不含空格；它同时就是这个进程的进程名，用来命名相关的 worktree / 分支）、`title`（不可留空的一行摘要 ≤200，`process list` / `inspect` 显示的就是它）、`detail`（任务详情正文，可多行、可以为空 ≤20000，为空时以 title 为准）。`goal` 仍然是「要实现什么」的简述，`detail` 是它的展开；`name` / `title` 缺失或格式不合规时创建直接失败（-32602），报错引述该变量的声明。工作参数（仓库路径、分支）仍然由任务自己向父进程取（`project` 的 `path` / `branch`），所以 `dev-task` 不声明 `path`。它唯一的子模板是 `worktree-service`（于是它不再是叶子）：它的 `system_prompt` 写明「改文件不去主工作树」的协议——先 `git -C <仓库> worktree add <目录> -b <分支>`（目录默认取仓库同级的 `<仓库名>-<name>`，跟随仓库既有约定，不建在仓库内部），确认目录存在，再用 `variables.path` 把这个 worktree 绑到 `worktree-service` 上并 call 它干活，自己不写业务代码、不 merge / rebase / 删 worktree。想给子进程带信息，可以直接像 `project` 那样声明变量，也可以像 `dev-task` 这样让子进程自己去父节点取（更灵活，但子进程必须真的去取）。

`worktree-service`（`dev-task` 的子模板、service、非单例、`child_templates: []`）是「给当前项目开的 git worktree 的管理器」：它只声明一个变量，就是保留名 `path`（必填、不可变）——该 worktree 的绝对目录，必须已经存在（先 `git worktree add` 再创建），核心的 `checkWorkdir` 会拒绝缺失 / 相对 / 不存在的值（-32602）；同一个保留名同时把它变成这个 service 的 agent 工作目录（`src/agent/invocation.js` 从 `state.params.path` 取 cwd），所以它一被 call 就站在那个 worktree 里，不需要 `cd`。`singleton=false`，但约定是「一个 worktree 一个管理器」：`spawn_prompt` 要求创建方先看自己的 `children`、有相同 `path` 的就复用。它的 `system_prompt` 让它先认清自己在管什么（`git rev-parse --show-toplevel` / `--abbrev-ref HEAD` / `git worktree list`，把 path、分支、主仓库写进 state），把改动限制在这一个 worktree 内，git 只做增量（`commit` 可以，push / rebase / `reset --hard` / 删分支 / `git worktree remove` 要先问），并且它是 Service：不 `complete`、不建子进程，被停掉前把当前状态写进 state。

`project` 的 `system_prompt` 是「收到一次 call 之后怎么做」的协议：先把输入拆成一条或多条任务（编号 / 项目符号列表、并列祈使句、互不依赖的交付物算多条；同一目标的多步骤、有先后的连续步骤不拆），每条的字段按目标模板自己的 `spawn_prompt` 与 `variables` 声明配置（不凭记忆，`dev-task` 给 `name` / `title` / `detail`，`research-task` / `generic-task` 只给 `goal`），用 `process_spawn` 一次创建多个 task 进程，并把拆分理由与 pid / name / title 写进自己的 state；默认只创建、不逐个 call。`project` 只创建 task 类子模板，`child_templates` 就是 `dev-task` / `research-task` / `generic-task` / `generic-service`：开 / 关项目是 `project-manager` 的职责（PID 0 → `project-manager` → `project`），嵌套 project 被白名单拒绝（-32010，`cannot create template project`），`available_child_templates` 里也不会出现 `project`。协议与所有提示词一样随实例快照冻结：改模板文件只对重启 daemon 后新建的 `project` 生效。
