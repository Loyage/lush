...
「待你决定」面板提供未处理、已回答、已忽略和全部记录四个筛选，包括普通提问、问卷、计划审批与纯提醒。点开后在面板内查看正文、答复或审批；已处理事项只读展示原问题与结果，不会因答复、忽略、刷新或重启丢失。原有显式 `task.delete` / `task.clear` 的清库行为不变，见[维护](maintenance.md)。
