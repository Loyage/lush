...
- 推荐项排第一并在 label 标「（推荐）」，默认**不自动选择**；显式开启[睡觉模式](../../sleep-mode.md)后可由管家按授权代理选择。多选仅用于多个选项可以同时成立的题目。
...
服务端从已存问卷生成规范化答案：`{version:1,answers:[{question,header,selected,labels,custom}]}`，JSON 存入 `notice.answer`。收件箱消息包含 `{notice_id,title,dismissed,answer}`，其中 answer 是规范化对象，不是客户端自报的标签。答复 RPC 只允许用户；显式授权的[睡觉模式管家](../../sleep-mode.md)由运行时内部代答并单独留档；同一 notice 只能结算一次。
