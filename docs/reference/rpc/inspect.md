...
`task.transcript_latest` 是同一批会话文件的**最新优先**只读读面：它不复用 `task.transcript` 的 8 MiB 头预算，而是对全部会话做一次完整的异步流式扫描，返回满足 `seq > after` 且（`before` 为 0 或 `seq < before`）的最新 `limit` 步（默认 100，最大 200），按 `seq` 升序；每步字段、4,000 字符裁剪与 `tokens` 口径与 `task.transcript` 完全一致。额外返回 `next`（窗口最大 `seq`）/ `oldest`（窗口最小 `seq`）/ `has_older`（`(after, oldest)` 内是否还有步骤），用 `oldest` 作下一页 `before` 可连续往回翻。单行 JSONL 超过 16 MiB 时跳过该行并置 `truncated`，不冒充空结果；内存只保留有界窗口与当前 token 批次，不缓存全部正文。全量扫描与全文搜索同级，超长历史不是恒定时间读取。
