...
`daemon start/restart` 是客户端工作流，不是 RPC。`doctor` 检查本地项目配置与 daemon 身份。`web [port]` 后台启动独立 Web 进程（日志 `.lush/web.log`），命令等它真的占住端口就返回；同一端口已经有 Lush Web 时幂等报告「已在运行」，不换进程。默认只监听本机，存在 `.lush/web.json` 时改为公网监听并启用登录认证。`web-restart [port]` 先停掉端口上那个后台 Web 再按当前代码起一个新的：Web 不跟着代码换版本，改完 `src/ui/web/` 之后用它。`web-stop [port]` 停掉后台 Web，`web-status [port]` 报告它在不在跑、跑的是不是这份代码与日志路径。停只能停命令行确实是 Lush Web 的进程，别的程序占着端口时报出它的命令行交还给你。`web --foreground`（即 `bin/lush-web`）占住终端，只在调试时用。
