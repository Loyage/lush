...
核心区别：**Task `completed` 只表示 Agent 交付了提交；Candidate `pending` 表示私有集成已经冻结为待验收结果，但尚未启动验收；`ready` 表示固定 commit 已有结构化 `pass` 证据与人类报告，但仍须用户显式接受；只有 Candidate `integrated` 才表示代码已经进入目标分支。**
