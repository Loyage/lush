...
say Task 可以预约**效果展示**或**合并请求**，二选一；同类预约可复查阻塞原因。点击预约展示即创建展示子 Task 并让它先做准备；say 真正完成且满足展示准入后，runtime 发信号让它按最终固定提交交付，say 在展示结算后才终结。展示结果不是检验通过或合并批准。合并请求须等任务静息、子任务结清、工作区干净且能快进，才冻结源 commit 与父分支基线并通知父 Task。请求发出后源 Task 可以结算，但父分支不会自动前进。
