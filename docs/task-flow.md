...
say Task 可以预约**效果展示**或**合并请求**，二选一；同类预约可复查阻塞原因。点击预约展示即创建展示子 Task 并让它先做准备；say 真正完成且满足展示准入后，runtime 发信号让它按最终固定提交交付，say 在展示结算后才终结。展示结果不是检验通过或合并批准。展示交付会把原 say 结算为 `completed`，此时它不再有可等的 pending 阶段；在分支图或详情点「请求合并」会直接补发一次固定提交请求（撤销后可再次请求），之后仍由父 Agent 或用户批准。若此时分支已与直接父分支分歧，请求不会直接发出，而是留下 pending/diverged 预约；用户可派一个不挂在终态 say 子树下的独立解分歧子 Task 吸收当时固定的父提交，完成后由 runtime 把产物快进推进回 say 分支并重新发出请求，再按固定 commit+baseline 批准。合并请求须等任务静息、子任务结清、工作区干净且能快进，才冻结源 commit 与父分支基线并通知父 Task。请求发出后源 Task 可以结算，但父分支不会自动前进。
