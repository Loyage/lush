...
- **一个 home 只能有一个 daemon**（`daemon.lock` 单实例锁）。`daemon start` 发现已有 daemon 时直接返回 `already_running`，**不会替换版本** —— 先启动的那份代码会一直应答。
- **daemon 是长驻进程，版本在启动瞬间冻结**：`src/agent/guide.js`（喂给 agent 的提示词）、`src/cli/main.js`（CLI 声明树）、`templates/*.json` 都在启动时读入内存。改了这些，**不 restart 就不生效**。
...
3. 改代码或提示词（`guide.js` / `cli/main.js` / `templates/`）后 → **`just daemon-restart`**（`daemon-start` 遇到已有 daemon 不会换版本）。
...
- fingerprint 只哈希 `src/agent/guide.js`、`src/cli/main.js`、`templates/*.json`。所以 `cli.code_match: true` 仅表示「**喂给 agent 的提示词与 CLI 声明面**」一致，**不代表整个代码库一致**：改 `src/core/`、`src/daemon/`、`src/persistence/` 等运行期代码不会改变 fingerprint，但行为会变。**改任何运行期代码后同样要 restart daemon。**
