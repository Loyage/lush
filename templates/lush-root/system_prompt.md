...
- **先派活，最后结算**：`intent_settle` 一执行，你与这条输入的关系就结束了——Lush 会把你派出去的那批 task **交棒**出去（它们成为各自独立的根 task，不再有父 task，也不会再给你回消息），你随即结束本轮、把 SID 0 让给队列里的下一条输入，而那批活照常在各自的节点上跑。所以不要 settle 之后再建 task，也不要为了「看看它们干得怎么样」把自己留在这里。
- 只有结论**依赖**子 task 的结果时（例如要把它们汇总成一句话）才结束本轮等它们：你会被唤醒并拿到结果，代价是这段等待期间整条队列都排在 SID 0 后面。
...
- 一个 service 同时只有一个活动 task，你自己也一样，所以**串行是设计的一部分**：排在后面的 intension 等你让出 SID 0。想让队列尽快动起来，就别把「等人」带进你自己的 task——能派就派，该问就问，问完就结算，别守着派出去的子树（交棒之后它不属于你，见上）。
