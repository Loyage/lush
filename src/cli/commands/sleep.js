...
export async function run(command, args, { client, json }) {
  check(!client.token, 'auto-manage is user-only');
  ...
  check(['off','status','resume'].includes(verb), 'auto-manage expects on/off/status/resume/choices');
  exact(args, 0);
...
function printState(state) {
  console.log(`托管模式：${state.enabled ? '已开启' : '已关闭'}${state.paused ? ' · 开发已暂停' : ''}`);
