 ...
 *
 * A process is a passive node; a task is one piece of work mounted on it, and
 * it is what has an agent. `lush call PID '...'` creates the root task of a
 * collaboration; the tasks that root opens on its child processes (with
 * `task_spawn`) are how the work travels down the tree — `lush task tree`
 * shows that tree.
 */
export const taskGroup = {
  summary: 'Task：挂在 process 上的一次工作（有 agent、有 result、有子 task）',
  cover: [
    'task 是唯一会调用 agent 的东西：一个 task 有 goal、status、自己的草稿 state 和 result；它结束后 result 就是交给派活方的答案。',
    'task 只能挂在 process 上；一个 process 同时最多一个活动 task（下游正忙时派活会被拒绝）。子 task 只能挂在自己的直接子 process 上，所以 task 树总是沿着 process 树向下生长。',
    ...
    '不覆盖：被动节点本身（建、启停、变量、删除）见 `lush process`；用户入口 `lush call` 是「在某个 process 上开一个根 task 并等它」。',
  ],
  notes: [
  ...
  children: {
    list: {
      ...
      cover: [
        '一行一个 task：ID、挂载的 PID、父 task、状态、goal 摘要、result 摘要。',
        '--pid 只看挂在某个 process 上的 task；--status 只看某个状态；--roots 只看根 task（用户直接开的），--children 只看派出去的子 task。',
      ],
      usage: ['lush task list [--pid PID] [--status STATUS] [--roots|--children] [--limit N]'],
      options: {
        '--pid': { arg: 'PID', desc: '只看这个 process 上的 task', apply: (r, v) => { r.pid = intArg(v, '--pid'); } },
        '--status': {
          arg: 'STATUS',
    ...
    tree: {
      ...
      cover: [
        '每个节点一行：`#ID process-name[PID] status · goal 摘要`，下一层是它派出去的 task。这就是「一件事如何在进程之间合作完成」的视图。',
    ...
    inspect: {
      ...
      cover: [
        'task 本身（goal、status、result、error、草稿 state）、它挂载的 process、父 task、直接子 task、最近调用与事件。',
        '文本按「task 摘要 → process → 子 task → calls → events」分节打印；--json 给出完整快照。',
      ],
    ...
    cancel: {
      ...
      cover: [
        ...
        '已结束的 task 幂等返回，不会复活。取消一个 process 上的活动 task 之后，才能 `lush process stop` 它。',
      ],
    ...
    spawn: {
      command: 'task_spawn',
      method: 'task.spawn',
      summary: '在某个 process 上派一个 task（不等待）',
      cover: [
        '在 PID 上创建并立刻启动一个 task，返回它的 id；随后用 `task wait` / `task inspect` / `task tree` 观察。',
        '这是 agent 内部 `task_spawn` 工具的命令行等价物：--parent-task-id 给定时，PID 必须是父 task 所在 process 的直接子进程（下游委托）；不给时创建的是根 task。',
      ],
      notes: [
        '同一个 process 同时只能有一个活动 task；该 process 正忙时派活会被拒绝。',
      ],
      usage: ['lush task spawn PID --goal GOAL [--parent-task-id TASK_ID]'],
      positionals: [['PID', '挂载 task 的 process PID']],
      options: {
    ...
    'update-state': {
      ...
      cover: [
        '把 --patch 的顶层字段 shallow-merge 进 task.state：这是「这一次工作」的进展记录，进程的长期 state 是另一份（`lush process update-state`）。',
      ],
    ...
    session: {
      ...
      cover: [
        ...
        'task 的 agent 属于这个 task：session 按 task 分开，多个 task 即使挂在同一个 process 上也不会共用会话。',
        '--open 用带着 Lush 身份与环境（LUSH_PID / LUSH_TASK_ID）的命令把当前终端交给 pi TUI 接续该会话；内置运行时没有外部 session，会报错。',
      ],
      notes: [
    ...
    delete: {
      ...
      cover: [
        '只删 task 行与它的事件；messages / agent_calls 作为 process 的历史保留（task 是组织单位，历史留痕在 process 上）。',
