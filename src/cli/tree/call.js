...
/**
 * `lush call PID GOAL` — the user's entry point.
 *
 * It creates a **root task** on the process and waits for it: the task's agent
 * works the goal, delegating to child processes with child tasks as needed, and
 * the command returns the task tree's outcome. That is the whole shape of Lush
 * in one command — a process (passive node) receives a task (work), and the
 * work travels down the process tree until it is done.
 */
export const callCommand = {
  command: 'call',
  method: (args) => (args.dry_run ? 'call.describe' : 'call'),
  summary: '在某个 process 上创建 task 并等它（及其子树）结束',
  cover: [
    '在 PID 上创建一个根 task，立刻开始跑它的 agent；命令阻塞到该 task 进入终态，返回 task 快照（含 result）。因为终态 task 不会有活动子 task，这也等于等到它派出去的一整棵子树结束。',
    '想先拿到 task id 就走，用 --detach：返回 task 快照，随后用 `lush task wait` / `task inspect` / `task tree` 观察（`lush task tree TASK_ID` 能看到这件事在进程间怎么协作）。',
  ...
  notes: [
    ...
    '要求目标 process 为 active，且它身上此刻没有别的活动 task（一个 process 同时只做一件事）；被拒绝时先 `lush task wait` 那个 task，或换一个下游节点。',
  ...
  usage: [
    'lush call PID GOAL [--detach]',
    'lush call PID GOAL --interactive  # 在本终端进入 pi TUI 做这个 task',
    'lush call PID GOAL --dry-run      # 只描述将要执行的调用',
  ],
  positionals: [['PID', '目标进程 PID'], ['GOAL', '要做什么（原话，会成为 task 的 goal）']],
  options: {
    '--detach': {
      arg: null,
  ...
  parse: (args) => ({ pid: intArg(args.shift(), 'pid'), goal: next(args, 'goal') }),
  check: (r) => {
