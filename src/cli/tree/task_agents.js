...
export const taskAgentGroup = {
  ...
  cover: [
    'agent 是「此刻在替某个 task 干活」的工作者，不是逻辑进程也不是 task：它没有自己的 pid，只有 agents 空间的 id `TASK.N`（TASK 是它服务的 task，N 是本次 daemon 内该 task 的第几个 agent）。',
  ...
  notes: [
    ...
    '一个 task 同时最多一个活动 agent；一个 process 同时最多一个活动 task，所以一个 process 上此刻也只可能有一个 agent。',
  ],
  children: {
    list: {
      ...
      cover: [
        '按 `TASK.N` 升序列出：AGENT、TASK、PID、NAME、PROVIDER、STATUS、CALL、OS-PID、ELAPSED、MODE。',
      ...
      notes: [
        ...
        '--task-id / --pid 只看某个 task 或某个 process 的 agent；--pid 看到的是挂在该 process 上的那个活动 task 的 agent。',
      ],
      usage: ['lush task agents list [--task-id TASK_ID] [--pid PID] [--all]'],
      options: {
        '--task-id': { arg: 'TASK_ID', desc: '只看该 task 的 agent', apply: (r, v) => { r.task_id = intArg(v, '--task-id'); } },
        '--pid': { arg: 'PID', desc: '只看该 process 上的 agent', apply: (r, v) => { r.pid = intArg(v, '--pid'); } },
    ...
    show: {
      ...
      cover: [
        '给出该 agent 的 id、task_id、pid、provider、status、call_id、os_pid、interactive、cancellable、开始/结束时间、时长与 error（若有）。',
