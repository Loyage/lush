...
- \`daemon ...\`：daemon 自身的启停与状态（\`start\` / \`stop\` / \`restart\` / \`status\`）。daemon 未运行时，除 \`lush daemon start\` / \`lush daemon restart\` 与 \`lush agent ...\` 外的命令都会连接失败（退出码 1）。改完代码或提示词用 \`lush daemon restart\`（只影响本次 LUSH_HOME 那一份 daemon）。
