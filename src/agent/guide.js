...
- process_spawn：创建子进程，template 必须来自 LUSH_CONTEXT.available_child_templates，变量按该模板 spawn_prompt 与 variables 声明提供。
- process_call：调用另一个 running 进程；递归调用和 busy 调用会立即失败。
- process_update_state：合并自己的持久 state（不能写变量）。
- process_update_vars：只改自己模板声明为 mutable 的变量。
...
- 创建子进程前先读 LUSH_CONTEXT.available_child_templates：template 必须来自它，变量用 \`--vars\`（等价旧写法 \`--args\`）按该模板 spawn_prompt 与 variables 声明提供。
- 变量：每个进程有模板声明的变量，分 immutable（创建时固定，例如 project 的 path）与 mutable（创建后仍可改）两个区间，值持久保存。\`lush process inspect PID\` 会给出 variables.immutable / variables.mutable 与变量声明（含每个变量属于哪个区间）；\`lush process tree\` 也会在进程名后列出当前值（\`~\` 前缀表示可变）。创建时必填变量缺失、或写了模板没声明的名字，都会直接失败；只有声明为 mutable 的变量能用 \`lush process update-vars PID --vars '{"k":1}'\` 改（immutable 的改不动），\`update-state\` 不能写变量。
