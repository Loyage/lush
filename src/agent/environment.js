...
export function agentEnvironment(config, role) {
  ...
  const files = [path.join(dir, 'agent.env'), path.join(dir, `${role}.env`)];
  const values = {};
  const sources = [];
  ...
  check(bytes <= 65536, `assembled ${role} agent environment exceeds 65536 bytes`);
  return { role, values, sources, files };
}
