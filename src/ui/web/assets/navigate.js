 ...
 */
let navigation = { refresh: async () => {}, detail: async () => {}, overview: async () => {}, graph: async () => {}, resource: () => false };

export function registerNavigation({ refresh, detail, overview, graph, resource }) {
  const previous = navigation;
  const registered = { refresh, detail, overview, graph, resource: resource ?? previous.resource ?? (() => false) };
