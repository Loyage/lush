...
function readableText(value, { markdown = false, preview = true, key, expanded = false } = {}) {
  ...
  const render = full => {
    ...
    content.replaceChildren(markdown ? agentText(shown, { plain: 'pre', className: 'transcript-prose' }) : el('pre', shown, 'readable-value'));
  };
...
export function transcriptBody(step, { key = '', preview = true } = {}) {
  const root = el('div', undefined, 'transcript-content');
  ...
  const field = (name, item, suffix = name) => {
  ...
  for (const [name, item] of entries.slice(0, 40)) {
    if (name === 'edits' && Array.isArray(item)) {
      item.slice(0, 20).forEach((edit, index) => {
        ...
        for (const [part, content] of Object.entries(edit).slice(0, 40)) change.append(field(part, content, `edit:${index}:${part}`));
        root.append(change);
      });
      if (item.length > 20) root.append(el('p', '仅预览前 20 处修改；其余见原文。', 'hint'));
    } else root.append(field(name, item));
  }
