...
命令分两层组：命令组（`daemon` / `process` / `task` / `agent`）与顶层入口 `call` → 命令 → 参数。默认输出是给人读的文本（表格、树、分块的 message、一行式状态），`--json` 给出稳定的机器可读 result，可写在命令之前或末尾。运行期 agent 属于 task（`lush task agents …`），不是独立的命令层。
...
just call 2 'hi'     # 在 PID 2 上开一个根 task 并等它结束（just call 2 'hi' dry 只打印命令）
...
just reset yes       # 只清进程树（daemon、日志、session 都保留），不可逆
```

完整清单、`just clean` 与 `just reset` 的区别、以及每个命令的参数，见 [docs/reference/cli.md](docs/reference/cli.md)。

**改代码或提示词之后，先确认你重启的是哪个 daemon**：daemon 是长驻进程，`start` 不会替换版本，只有 `restart` 会，而且只重启 `LUSH_HOME` 指向的那一份。`lush daemon status`（或 `just doctor`）会列出 daemon 与 CLI 各自的 `home` / `code_dir` / `fingerprint`，不一致时任何 `lush` 命令都会在 stderr 上告警。判据与排障见 [docs/engineering/identity.md](docs/engineering/identity.md)。
...
lush process spawn 1 project x --vars '{"path":"/abs/repo"}'   # 进程也可以用 --agent 指定 profile
```

字段表、选择优先级（进程 > 环境变量 > 内置 default）、`mock` / `openai` 后端与 session 的位置，见 [docs/reference/agents.md](docs/reference/agents.md) 与 [docs/concepts/agents.md](docs/concepts/agents.md)。

## 文档

读哪一份取决于你要回答什么；每份文档都在开头写了自己的定位。

- **概念**：[Process 与 Task 模型](docs/concepts/process-model.md) · [生命周期与孤儿监督](docs/concepts/lifecycle-and-orphans.md) · [Agent 后端与 Context](docs/concepts/agents.md)
...
**生命周期提示：** Process 只有 created / active / stopped——它是被动的，`process spawn` 只是建节点，不会跑任何 agent，`process stop` 只让它不再接受 task（先取消它手上的 task）。工作全在 task 上：`call` 建根 task 并等待，task 的状态是 created / running / waiting / completed / failed / cancelled，`task cancel` 取消一棵子树，`task complete` 由它的 agent（或人）在目标达成时调用；终态 task 不会有活动子 task。节点结束时（`stop` / `purge`），活动的直接子节点改挂 PID 0。**删除是唯一的物理删除路径**：`task delete` 只删 task 行（call 行与消息留作 process 的历史），`process delete PID` 只删 stopped 且没有活动 task 的节点（连带它上面的 task），`process purge PID` 先取消 task、停止节点再删（`--recursive` 连整棵子树），且内置运行时的 agent 工具集里没有删除工具；被删节点的父进程会得到一条 `child_deleted` 事件。PID 0 永远拒绝，只能通过停止 daemon 退出。详见 [docs/concepts/lifecycle-and-orphans.md](docs/concepts/lifecycle-and-orphans.md)。

本地单用户 MVP：没有 ACL、沙箱、自动调度、自动任务恢复、向量数据库或 Web UI。运行 pi 时，pi 自带的 read/bash/edit/write 工具和你的 pi 配置（skills、extensions、AGENTS.md）都会生效，因此 pi 进程能读写磁盘和执行命令；`openai` / `mock` 运行时只有 Lush 自己的 `process_*` / `task_*` 工具，不含 shell、文件编辑或联网能力。不要向不可信用户暴露 socket；Agent 可以调用其他 Process，因此工具调用不是安全隔离边界。
